...
error  <- matrix(nrow = 10, ncol = 1)
  ...
  write.csv(internal, "internal.csv", row.names=TRUE)
  write.csv(external, "external.csv", row.names=TRUE)
  ...
  #data[,-ncol(data)]  # Data set without the activity label
  test = data[,-ncol(data)]
  write.csv(internal, "internal_without_label.csv", row.names=TRUE, na="")
  ...
  internal = read.csv("internal.csv", header = TRUE)
  rownames(internal) <- internal$X
  within(internal, rm("X"))

  data = read.csv("external.csv", header = TRUE)
  rownames(external) <- external$X
  within(external, rm("X"))

  Model = randomForest(Activity ~ ., internal, ntree= 500,mtry =100)


  results = data.frame(Prediction= predict(Model,test), round(predict(Model,test,type="prob"),3))
  print(results)
